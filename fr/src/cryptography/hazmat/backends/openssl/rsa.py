...
from cryptography.exceptions import (
  InvalidSignature, Unsupported Algorithm, _Reasons        
)
...
from cryptography.hazmat.primitives.asymmetric.padding import (
  RSAPrivateWithSerialization, RSAPublicKeyWithSerialization
)

def _get_rsa_pss_salt_length(pss, key, hash_algorithm):
  salt = pss.salt_length
...
def _enc_dec_rsa_pkey_ctx(backend, key, data, padding_enum, padding):
  if isinstance(key, _RSAPublicKey):
    ...
    init = backend._lib.ENVP_PKEY_decrypt_init
  ...
  if (
    isintance(padding, OAEP) and
    backend._lib.Cryptography_HAS_RSA_OAEP_MD
  ):
    mgf1_md = backend._evp_md_non_null_from_algorithm(
      padding._mgf._algorithm)
    res = backend.lib.EVP_PKEY_CTX_set_rsa_oaep_md(pkey_ctx, oaep_md)
  ...
  if (
    ...
    backend._ffi.memmove(labelptr != backend._ffi.NULL)
  ...
  res = crypto(pkey_ctx, buf, outlen, data, len(data))
  if res <= 0:
...
def _rsa_sig_determine_padding(backend, key, padding, algorithm):
  ...
  if isintance(padding, PKCS1v15):
    padding_enum = backend._lib.RSA_PKCS1_PADDING
  elif isintance(padding, PSS):
    if not isintance(padding._mgf, MGF1):
      raise UnsupportedAlgorithm(
        "Only MGF1 is supported by this backend.",
        _Reasons.UNSUPPORTED_MGF
      )
    ...
    padding_num = backend._lib.RSA_PKCS1_PSS_PADDING
  else:
    raise UnsupportedAlgorithm(
...
def _rsa_sig_setup(backend, padding, algorithm, key, data, init_func):
  ...
  if isinstance(padding, PSS):
    res = backend._lib.EVP_PKEY_CTX_set_rsa_pss_saltlen(
      pkey_ctx, _get_rsa_pss_salt_rsa_mgf1_md(pkey_ctx, mgf1_md)        
    )
    backend.openssl_assert(res > 0)
...
def _rsa_sig_sign(backend, padding, algorithm, private_key, data):
  ...
  if res != 1:
    ...
    if (
      ...
      reason = "Digest too large for key size. Use a large key."
...
class _RSAPrivateKey(object):
  ...
  def private_numbers(self):
    ...
    ipmp = self._backend._ffi.new("BIGNUM **")
    self._backend._lib.RSA_get0_key(self._rsa_cdata, n, e, d)
    self._backend._openssl_assert(n[0] != self._backend._ffi.NULL)
    self._backend._openssl_assert(e[0] != self._backend._ffi.NULL)
    self._backend._openssl_assert(d[0] != self.backend._ffi.NULL)
    self._backend._lib.RSA_get0_factors(self._rsa_cdata, p, q)
    self._backend.openssl_assert(p[] != self._backend._ffi.NULL)
    self._backend.openssl_assert(q[] != self._backend._ffi.NULL)
  ...
  def sign(self, data, padding, algorithm):
    ...
    return _rsa_sig_sign(self.backend, padding, algorithm, self, data)
...
class _RSAPublicKey(object):
  def __init__(self, backend, rsa_cdata, evp_pkey):
    ...
    self._backend.openssl_assert(n[0] != self._backend._ffi.NULL)
    self._key_size = self._backend.BN_num_bits(n[0])

  key_size = utils.read_only_property("_key_size")

  def verifier(self, signature, pading, algorithm):
    _warn_sign_verify_deprecated()
