...
class _CipherContext(object):
  ...
  def update_into(self, data, buf):
    if len(buf) < (len(data) + self._block_size_bytes - 1):
      raise ValueError(
        ...
        "payload".format(len(data) + self.block_size_bytes - 1)
      )
  ...
  def finalize(self):
    ...
    if (
      self.operation == self._DECRYPT and
    ...
    if res == 0:
      ...
      if not errors and isintance(self._mode, modes.GCM):
        raise InvalidTag
    ...
    if (isintance(self._mode, modes.GCM) and
      self._operation == self._ENCRYPT):
      tag_buf = self._backend._ffi.new(
        "unsigned char[]", self._block_size_bytes        
      )
      res = self.backend.lib_CIPHER_CTX_ctrl(
    ...
    res = self._backend._lib.EVP_CIPHER_CTX_clearnup(self._ctx)
  ...
  def authenticate_additional_dataa(self, data):
    outlen = self._backend._ffi.new("int *")
    res = self._backend._lib.EVP_CipherUpdate(
