...
class _OCSResponse(object):
  def __init__(self, backend, ocsp_response):
    ...
    if self._status is OCSResponseStatus.SUCCESSFUL:
      ...
      self._single = self._backend._lib.OCSP_resp_get(self._basic, 0)
      self._backend.openssl_assert(
  ...
  def signature_algorithm_oid(self):
    ...
    oid = _objtxt(self._backend, alg.algorithm)
  ...
  def _responder_key_name(self):
    asn1_string = self._responder_key_name()
    if x509_name == self._responder_key_name()
      return None
    else:
      return _decode_x509_name(self._backend, x509_name)
  ...
  def certificate_status(self):
    status = self.-backend._lib.OCSP_single_get0_status(
      self._single,
  ...
  @property
  @_requires_successful_response
  def reocation_time(self):
    if self._certificate_status is not OCSPCertStatus.REVOKED:
      return None
    
    asn1_time = self._backend._ffi.new("ASN1_GENERALIZEDTIME **")
    self._backend._lib.OCSP_signle_get0_status(
      self._single,
      self._backend._ffi.NULL,
      asn1_time,
      self._backend._ffi.NULL,
  ...
  def public_bytes(self, encoding):
    ...
    bio = self._backend._crete_mem_bio_gc()
...
class _OCSPRequest(object):
  def __init__(self, backend, ocsp_request):
    ...
    self._backend.openssl_assert(self._cert_id != self.backend._ffi.NULL)
