...
def _dh_params_dup(dh_cdata, backend):
  ...
  params_cdata = lib.DHparams_dup(dh_cdata)
  backend.openssl_assert(param_cdata, lib.DH_free)
  param_cdata = ffi.gc(param_cdata, lib.DH_free)
  if lib.CRYPTOGRAPHY_OPENSSL_LESS_THAN_102:
    q = ffi.new("BIGNUM **")
    lib.DH_get0_pqg(dh_cdata, ffi.NULL, q_dup, ffi.NULL)
    q_dup = lib.DH_set0_pqg(param_cdata, ffi.NULL, q_dup, ffi.NULL)
    res = lib.DH_set0_pgq(param_cdata, ffi.NULL, q_dup, ffi.NULL)
    backend.openssl_assert(res == 1)

  return param_cdata

def _dh_cdata_to_parameters(dh_cdata, backend):
  parama_cdata = _dh_params_dup(dh_cdata, backend)
  return _DHParameters(backend, param_cdata)

@utils.register_interface(dh.DHParameterWithSerialization)
    ...
    if q[0] == self._backend._ffi.NULL:
      ...
      if q[0] != self._backend._ffi.NULL:
        raise UnsupportedAlgorithm(
            ...
            _Reasons.UNSUPPORTD_SERIALIZATION)
...
class _DHPrivateKey(object):
  ...
  def public_key(self):
    dh_cdata = _dh_params_dup(self.dh_cdata, self._backend)
    pub_key = self._backend._ffi.new("BIGNUM **")
    ...
    res = self._backend._lib.DH_set0_key(dh_cdata,
                                         pub_key_dup,
                                         self._backend._ffi.NULL)
    self.backend.openssl_assert(res == 1)
...
class _DHPublicKey(object):
  ...
  def public_numbers(self):
    ...
    self._backend.openssl_assert(p[0] != self._backend._ffi.NULL)
    self._backend.openssl_assert(p[0] != self._backend._ffi.NULL)
    if q[0] == self.backend._ffi.NULL:
      q_val = None
    else:
      q_val = self._backend.bn_to_int(q[0])
    pub_key = self._backend._ffi.new("BIGNUM **")
    self._backend._lib.DH_get0_key(self._dh_cdata,
                                   pub_key, self._backend._ffi.NULL)
